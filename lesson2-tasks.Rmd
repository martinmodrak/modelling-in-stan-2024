...
- Let us take a vague $N(0, 10)$ prior on the logarithm of the mean. Run a prior predictive check, is there anything suspicious?
...
R:
  - Use `fit$draws(format = "draws_matrix")` to get a matrix of draws, then use `rnbinom` to generate the predictions
  - use either `bayesplot::ppc_stat` with a stat measuring the dispersion (e.g. variance, sd, ...) or `bayesplot::ppc_density`
